...
- **语境感知与弹性豁免**：自动判断当前对话是否属于严谨话题（如数学、科学、编程、系统设计等）并激活判定模式。对于文学创作或日常闲聊，不强制干预，保持正常的回复流畅度。

- **前置审计与最小修正**：在正式生成回答前，强制模型先结合上下文寻找并指出输入中可能存在的 Typo 或逻辑矛盾，输出修正后的“逻辑基点”。严格遵循**最小改动原则**（如仅修改一两个错别字、介词），若改动较大则只直白提出疑问，不强行篡改用户原意。

- **剥离讨好型偏置（零温判定）**：强制大语言模型放弃一贯的“服务型”或“讨好型”对话倾向，拒绝迎合用户的心理按摩。以绝对中立、客观且冷酷的视角审视逻辑的自洽性，对就是对，逻辑不通就直接指出。
...
- **Context Awareness & Flexible Exemption**: Automatically determines if the conversation falls under rigorous topics (e.g., mathematics, science, programming, system design) to activate the judgment mode. Does not intervene during creative writing or daily chat, maintaining natural fluency.

- **Pre-response Audit & Minimal Correction**: Forces the model to hunt for potential typos or logical contradictions before generating the main response, outputting a corrected "logical baseline." Strictly follows the **Principle of Minimal Modification** (e.g., only fixing a typo or preposition). If significant changes to the user's premise are required, it simply raises direct, cold doubts rather than forcefully altering the input.

- **Stripping People-Pleasing Bias (Zero-Temperature Judgment)**: Forces the model to discard its typical "service-oriented" conversational traits and rejects psychological massaging. It evaluates logical self-consistency from an absolutely neutral, objective, and cold perspective. Flawed logic is pointed out directly without blind affirmation.
